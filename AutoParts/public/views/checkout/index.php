...
<section class="py-4">
    ...
    <div class="row g-4">
        ...
        <div class="col-12 col-lg-7">
            <div class="card border-0 shadow-sm rounded-4 h-100">
                ...
                <div class="card-body fs-6">
                    ...
                    <form action="/checkout/submit" method="post" class="row g-3" id="checkout-form">
                        <?= \App\Core\Csrf::csrfInput(); ?>

                        <!-- hidden: чи треба одразу LiqPay -->
                        <input type="hidden" name="pay_with_liqpay" id="pay_with_liqpay" value="0">
        ...
        <!-- ПРАВА КОЛОНКА: ПІДСУМОК ЗАМОВЛЕННЯ + LiqPay КНОПКА -->
        <div class="col-12 col-lg-5">
            <div class="card border-0 shadow-sm rounded-4">
                ...
                <div class="card-body p-0">
                    <ul class="list-group list-group-flush">
                        <?php foreach ($cart['items'] as $item): ?>
                            ...
                            $sum         = $price * $qty;
                            ?>
                            <li class="list-group-item d-flex justify_content-between align-items-start">
                                <div class="me-2">
                ...
                <!-- Блок з кнопкою LiqPay -->
                <div id="card-pay-block" class="card-footer bg-white border-0 d-none">
                    <button type="button" class="btn btn-success w-100 mb-1" id="card-pay-btn">
                        <i class="bi bi-credit-card me-1"></i>
                        Оплатити карткою (LiqPay)
                    </button>
                    <div class="small text-muted">
                        Після натискання замовлення буде оформлено і ви перейдете на сторінку оплати LiqPay.
                    </div>
                </div>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    var cardBlock       = document.getElementById('card-pay-block');
    var cardBtn         = document.getElementById('card-pay-btn');
    var checkoutForm    = document.getElementById('checkout-form');
    var payWithLiqpayEl = document.getElementById('pay_with_liqpay');

    if (!paymentSelect || !cardBlock || !checkoutForm || !payWithLiqpayEl) return;

    function updateCardBlockVisibility() {
        var selectedOption = paymentSelect.options[paymentSelect.selectedIndex];
        if (selectedOption && selectedOption.dataset.online === '1') {
    ...
    paymentSelect.addEventListener('change', updateCardBlockVisibility);
    updateCardBlockVisibility(); 

    if (cardBtn) {
        cardBtn.addEventListener('click', function () {
            
            payWithLiqpayEl.value = '1';
            checkoutForm.submit();
        });
    }
});
