...
class Category extends Model
{
    ...
    {
        // Готуємо query-параметри для API
        $params = [];

        if ($search !== null && $search !== '') {
            // На Node ми приймаємо search або q → тут відправляємо search
            $params['search'] = $search;
        }

        try {
            // Другий аргумент піде як ?search=... до /api/categories/{slug}/children
            return self::getList(
