...
export async function applyCouponToCart(cartId, couponCode) {
  ...
  // 1) Беремо кошик з БД (price / quantity НЕ міняємо)
  const cart = await getCart(cid);
  if (!cart || (cart.total || 0) <= 0) {
  ...
  // 2) Перевіряємо купон відносно поточної суми
  ...
  // 3) Рахуємо суму до оплати, але БД НЕ чіпаємо
  const totalWithDiscount = Math.max(0, totalBefore - discount);

  return {
    ...cart,                           // items з цінами як у БД
    discount,                          // загальна знижка
