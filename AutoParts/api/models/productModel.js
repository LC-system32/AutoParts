// file: API/models/productModel.js
...
function buildProductFilters(filters = {}) {
  ...
  // 🔍 Фільтр по категорії (slug категорії або її parent)
  if (category) {
    where.push(`(
      c.slug = $${idx}
      OR parent.slug = $${idx}
  ...
  // 🚗 ПІДБІР ПО АВТО ЧЕРЕЗ product_fitments
  if (search_type === 'car') {
    // 1) Найточніший — по modification_id
    if (modification_id) {
      where.push(`EXISTS (
        SELECT 1
    ...
    // 2) Якщо модифікація не вибрана — по поколінню
    ...
    // 3) Якщо немає покоління — по моделі
    else if (model_id) {
      where.push(`EXISTS (
        SELECT 1
        FROM product_fitments pf
    ...
    // 4) Якщо тільки марка — по make_id
    else if (make_id) {
      where.push(`EXISTS (
        SELECT 1
        FROM product_fitments pf
...
export async function listProducts(filters = {}) {
  ...
  // page може прийти як число або строка
...
export async function createReview(productId, userId, ratingOrPayload, maybeComment) {
  // Випадок: третій аргумент – обʼєкт
  ...
  // Випадок: createReview(productId, userId, rating, comment)
